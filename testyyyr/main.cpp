...
void drawOblast()
{
    txRectangle (550, 20, 750, 70);    //область є вопроса
...
setlocale(LC_ALL, "Russian");
    ...
    while(n_question <= count_question)
    {
        ...
        sprintf(str, "¬опрос є%d из %d", n_question, count_question);
        ...
        if(ans != 0)
        {
        if(form.correct == ans) score++;
        if(form.correct != ans) break;
...
txSetFillColor (TX_BLACK);
txClear();
sprintf(str, "¬аш результат - %d из %d", score, count_question);
